...
> 来源: https://www.codebuddy.cn/docs/cli/security
...
- 沙箱化 bash 工具: Bash 沙箱隔离 bash 命令的文件系统和网络访问，减少权限提示的同时保持安全性。使用 /sandbox 启用以定义 CodeBuddy Code 可以自主工作的边界
...
- 输入清理：通过处理用户输入防止命令注入
- 命令阻止列表：默认阻止从网络获取任意内容的风险命令，如 curl 和 wget。当明确允许时，请注意权限模式限制
...
1. 批准前审查建议的命令
1. 避免将不受信任的内容直接传输到 CodeBuddy
1. 验证对关键文件的建议更改
1. 使用虚拟机(VM)运行脚本和进行工具调用，特别是在与外部 Web 服务交互时
...
可通过 CODEBUDDY_DISABLE_REQUEST_VALIDATION=1 关闭。详见 HTTP API 安全。


### 认证保护 ​

敏感端点（包括 /info、/health）在启用密码认证时需要 Bearer Token。详见 HTTP API 认证。
...
- 身份和访问管理 - 配置权限和访问控制
- Bash 沙箱 - bash 命令的文件系统和网络隔离
- MCP 集成 - 配置 MCP 服务器权限
- Hooks 文档 - 使用 hooks 进行自定义安全验证
- 设置配置 - 完整的配置选项
