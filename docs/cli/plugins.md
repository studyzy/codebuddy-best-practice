...
> 来源: https://www.codebuddy.cn/docs/cli/plugins
...
要安装已有的插件，请参阅插件市场。完整的技术规范请参考插件参考文档。
...
> 提示：先在 .codebuddy/ 中使用独立配置快速迭代，准备共享时再转换为插件。
...
- 已安装并认证 CodeBuddy Code

> 如果看不到 /plugin 命令，请将 CodeBuddy Code 更新到最新版本。参见故障排除获取升级说明。
...
更多字段如 homepage、repository 和 license，参见完整清单 Schema。
...
CodeBuddy 会按名字问候你。更多关于向技能传递参数的信息，参见 Skills 文档。
...
> --plugin-dir 参数适用于开发和测试。当你准备与他人共享插件时，参见插件市场。
...
插件可以包含代理技能来扩展 CodeBuddy 的能力。技能是模型调用的：CodeBuddy 会根据任务上下文自动使用它们。
...
安装插件后，运行 /reload-plugins 加载技能。完整的技能编写指南，包括渐进式披露和工具限制，参见代理技能。
...
插件 hooks/hooks.json 中的 hooks 在插件启用时自动与用户和项目级 hooks 合并（不会覆盖），并且不受 allowUntrustedFrontmatterHooks 闸门约束（该闸门只针对 Agent / Skill frontmatter 中声明的 hooks）。

除了 command，hook 也支持 type: prompt（小模型语义判定）、type: agent（subagent 验证）、type: http（POST/PUT/PATCH 到指定 URL）三种执行方式，详见 Hooks 文档。如果你的插件还需要随 Skill 一起携带 frontmatter hooks，请参考 Skills 文档 - 在 Skill 中配置 Hooks（注意此路径受安全闸门约束）。
...
设置 agent 会将插件的一个自定义代理激活为主线程，应用其系统提示词、工具限制和模型。这让插件在启用时可以改变 CodeBuddy Code 的默认行为。
...
> 可以通过多次指定参数同时加载多个插件：bashcodebuddy --plugin-dir ./plugin-one --plugin-dir ./plugin-two
...
1. 单独测试组件：分别检查每个命令、代理和钩子
1. 使用调试模式：用 --debug 参数启动 CodeBuddy 查看详细日志
...
1. 版本管理：在 plugin.json 中使用语义化版本
1. 创建或使用市场：通过插件市场分发
1. 与他人测试：在更广泛分发之前，让团队成员测试插件

插件上线到市场后，其他人可以按照插件市场中的说明安装使用。
...
1. 提供完整元数据：在 plugin.json 中提供详细的描述和作者信息
1. 版本管理：使用语义化版本号（Semantic Versioning）
1. 文档完善：为每个命令和技能提供清晰的描述
1. 测试充分：使用 --plugin-dir 在本地测试插件后再发布


### 安全注意事项 ​

1. 仅从可信源安装插件：插件可以执行命令和访问文件系统
1. 审查插件代码：安装前检查插件的命令和 Hooks
1. 使用权限控制：通过 CodeBuddy 的权限系统限制插件访问
...
- 插件市场 - 打包和共享你的插件
- 插件参考文档 - 完整技术规范
- 深入了解具体的插件组件： Skills - 技能开发详情子代理 - 代理配置和能力Hooks - 事件处理和自动化MCP - 外部工具集成
