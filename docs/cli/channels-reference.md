...
> 来源: https://www.codebuddy.cn/docs/cli/channels-reference
...
1. 发送 notifications/claude/channel 事件
1. 通过 stdio transport 连接（CodeBuddy Code 将其作为子进程启动）
...
- fakechat 示例：完整的双向 channel 实现，包含 Web UI、文件附件和 reply 工具
- MCP 协议：channel 基于 MCP 协议实现
