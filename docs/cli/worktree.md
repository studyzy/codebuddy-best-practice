...
> 来源: https://www.codebuddy.cn/docs/cli/worktree
...
1. 确定基础分支: 如果指定了 --worktree-branch origin/xxx，基于远程分支 origin/xxx如果指定了 --worktree-branch xxx，基于本地分支 xxx如果未指定，默认基于远程默认分支（通常是 origin/main 或 origin/master）
1. 自动创建对应的分支(如 worktree-feature-auth)
1. 将工作目录切换到 worktree；如果你是从仓库子目录启动，会优先进入新 worktree 中对应的相对子目录
1. 执行初始化(复制设置、创建符号链接、复制 .worktreeinclude 文件等)
...
与 Git worktree 不同，hook-based worktree 退出时始终显示 keep/remove 菜单，不做变更检测（因为非 Git 项目没有 git status）。
...
不是。子代理在文件层面隔离,但权限与主 Agent 相同,理论上可操作 worktree 之外的文件。这个功能的目的是防止多个 Agent 之间的文件冲突,而非安全限制。
