...
> 来源: https://www.codebuddy.cn/docs/cli/sub-agents
...
1. 打开子代理界面运行以下命令：/agents
1. 选择"创建新代理"选择是创建项目级别还是用户级别的子代理
1. 定义子代理推荐：先使用 AI 生成，然后自定义以使其成为您自己的详细描述您的子代理，包括何时应该使用它选择您想要授予访问权限的工具，或留空以继承所有工具界面会显示所有可用工具如果您使用 AI 生成，也可以按 e 在自己的编辑器中编辑系统提示
1. 保存并使用您的子代理现在可用。CodeBuddy Code 会在适当时自动使用它，或者您可以显式调用它：> 使用 code-reviewer 子代理检查我最近的更改
...
| tools | 否 | 特定工具的逗号分隔列表。如果省略，继承主线程中的所有工具。可使用 Defer(X) / NoDefer(X) 修饰符调整工具的延迟加载状态，详见 工具延迟加载覆盖 |
...
- 省略 tools 字段以继承主线程中的所有工具（默认），包括 MCP 工具
- 指定单个工具作为逗号分隔列表以获得更精细的控制（可以手动编辑或通过 /agents 编辑）
...
- 可用工具： Glob - 文件模式匹配Grep - 使用正则表达式进行内容搜索Read - 读取文件内容Bash - 仅限只读命令（ls、git status、git log、git diff、find、cat、head、tail）
...
> 说明：当工具输出过大时，完整输出会保存到 tool-results/ 目录中的文件，模型收到的是截断内容和文件路径指针，可以按需读取完整内容。详见环境变量参考。
...
- 插件 - 通过插件扩展 CodeBuddy Code 的自定义代理
- 斜杠命令 - 了解其他内置命令
- 设置配置 - 配置 CodeBuddy Code 行为
- Hooks - 使用事件处理程序自动化工作流
