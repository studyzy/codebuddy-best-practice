...
> 来源: https://www.codebuddy.cn/docs/cli/installation
...
> ⚠️ Beta 测试阶段原生二进制安装目前处于 Beta 测试阶段，功能仍在完善中。 如遇到任何问题，请在 Issues 页面 提交问题报告，或联系技术支持（codebuddy@tencent.com）。
...
1. 检查安装路径是否在 PATH 环境变量中：bashecho $PATH
1. 将 CodeBuddy 安装路径添加到 PATH（参考上方验证安装部分）
1. 重启终端或重新加载配置文件：bashsource ~/.bashrc  # 或 ~/.zshrc
...
这通常是系统中存在多个 codebuddy 可执行文件（如同时通过 npm 和 Homebrew 安装、nvm 切换了 Node 版本等）导致。详细排查步骤请参考 故障排查 - npm 安装成功但执行的仍是旧版本。
...
1. 检查网络连接
1. 配置 npm 镜像源（如果使用 npm 安装）：bashnpm config set registry https://registry.npmmirror.com
