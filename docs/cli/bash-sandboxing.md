...
> 来源: https://www.codebuddy.cn/docs/cli/bash-sandboxing
...
1. 减少权限提示：沙箱内的安全命令不需要批准
1. 保持安全性：访问沙箱外资源的尝试会立即触发通知
1. 实现自主性: CodeBuddy Code 可以在定义的限制内更独立地运行
...
CodeBuddy Code 包含一个有意设置的逃生舱机制，允许在必要时在沙箱外运行命令。当命令因沙箱限制（如网络连接问题或不兼容的工具）而失败时，CodeBuddy 会被提示分析失败，并可能使用 `dangerouslyDisableSandbox` 参数重试命令。使用此参数的命令会经过正常的 CodeBuddy Code 权限流程，需要用户权限才能执行。这允许 CodeBuddy Code 处理某些工具或网络操作无法在沙箱约束内运行的边缘情况。你可以通过在沙箱设置中设置 "allowUnsandboxedCommands": false 来禁用此逃生舱。禁用后,dangerouslyDisableSandbox 参数会被完全忽略,所有命令必须在沙箱中运行或明确列在 excludedCommands 中。
...
1. 沙箱已启用 (sandbox.enabled = true)
1. 自动批准配置已开启 (sandbox.autoAllowBashIfSandboxed = true)
1. 命令将在沙箱中运行 （不在 excludedCommands 列表中）
1. 未使用 dangerouslyDisableSandbox 参数
...
从 v2.90.x 起，在 bypassPermissions 权限模式下，此类沙箱越权审批会对非高危命令自动放行，避免 cd X && yarn build 这类日常组合命令反复触发审批：
...
1. 排除高风险命令：将 git push、rm -rf 等危险操作加入 excludedCommands
1. 定期审查：检查日志了解哪些命令被自动批准
1. 组合使用：配合 IAM 权限规则使用，提供深度防御
...
用户应该意识到允许广泛域名(如 `github.com`)可能带来的风险,这可能允许数据窃取。此外,在某些情况下,可能通过[域前置](https://en.wikipedia.org/wiki/Domain_fronting)绕过网络过滤。- 通过 Unix 套接字的权限提升: allowUnixSockets 配置可能会无意中授予对强大系统服务的访问权限,这可能导致沙箱绕过。例如,如果使用它允许访问 /var/run/docker.sock,这将通过利用 docker 套接字有效地授予对主机系统的访问权限。鼓励用户仔细考虑他们允许通过沙箱的任何 unix 套接字。
...
1. 监控日志：审查沙箱违规尝试以了解 CodeBuddy Code 的需求
1. 使用特定于环境的配置：为开发和生产环境使用不同的沙箱规则
1. 与权限结合：将沙箱与 IAM 策略一起使用以实现全面安全
1. 测试配置：验证你的沙箱设置不会阻止合法工作流程
1. 谨慎使用自动批准：只在充分理解沙箱限制的情况下启用 autoAllowBashIfSandboxed
