...
> 来源: https://www.codebuddy.cn/docs/cli/agent-teams


# Agent Teams：多智能体协作 ​

> Agent Teams 让你协调多个 CodeBuddy Code 实例组成团队协作，通过共享任务列表、成员间消息通信和集中式管理完成复杂任务。该功能目前存在已知限制。
...
1. CLI --subagent-permission-mode：本次会话所有 subagent 的默认权限
1. 环境变量 CODEBUDDY_SUBAGENT_PERMISSION_MODE：进程级覆盖
1. Settings permissions.subagentPermissionMode：全局或项目级配置
1. 继承主 session：默认继承领导的权限模式（但委派模式不继承，subagent 使用 default）
