...
> 来源: https://www.codebuddy.cn/docs/cli/models
...
| relatedModels | object | - | 关联模型配置，指定在不同场景（lite/reasoning/vision/longContext/subagent）下使用哪个模型 id。详见配置关联模型 |
...
> 通过 models.json 添加的自定义模型不会继承产品内置的 defaultRelatedModels。 如果没有在自身条目里显式声明 relatedModels，所有场景都会回退到主模型自己——也就是说子代理、lite、reasoning 全部用同一个大模型跑，成本和速度都不划算。
...
1. 当前主模型条目里的 relatedModels[variant]
1. 产品内置的 defaultRelatedModels[variant]（仅对内置模型生效，自定义模型跳过这步）
1. 回落到主模型自身
...
1. 确认 models 配置是否正确
1. 验证必填字段 （id, name, provider) 是否都已提供
