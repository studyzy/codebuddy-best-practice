...
> 来源: https://www.codebuddy.cn/docs/cli/permission-modes


# 权限模式 ​

> 控制 CodeBuddy 在编辑文件、运行命令、访问网络或调用其他高风险工具前，应该自动继续、询问用户，还是直接拒绝。权限模式决定的是会话节奏，不是整套权限系统的全部逻辑。
...
1. 命令安全检查（仅交互式）
1. ask 规则
1. bypassPermissions 短路
1. 不可信 allow 规则
1. 当前权限模式自己的基线策略
1. 非交互兜底与 auto / dontAsk 的最终收口
...
- dontAsk 不在键盘循环中，只能通过 CLI、settings、SDK / IDE 控制信号进入
...
- 只有当前 session 的计划文件写入是 plan 模式额外放行的特例
...
auto 不是“全自动放行”，而是把本来会 ask 的动作交给分类器做二次判断。
...
- 分类器出错 / 不可用：为安全起见直接拒绝该动作（fail-closed），并提示模型可改用其他自然工具达成、或停下来向用户说明；连续多次失败会自动退出 auto、回退到 default。
...
为了避免 allow 规则把分类器整个绕过，auto 模式下 CodeBuddy 会临时忽略“过宽 / 危险”的 allow 规则（只在本次判断时从内存过滤，不改写你的 settings）。被忽略的主要有：
...
auto 分类器的受信边界与放行 / 拦截规则由顶层 autoMode settings 控制（environment / allow / soft_deny / hard_deny），详见 Settings 配置。
...
- AskUserQuestion、ExitPlanMode 在 dontAsk 下也被拒绝（不弹框 / 不进入计划审批）——dontAsk 的本意就是“绝不打断用户”，所以连这类交互工具也不例外
...
CodeBuddy 的 plan 不是一个独立的“完全只读模式”，而是委托给进入 plan 前的模式：
...
默认情况下，子代理（Agent 工具调用、Agent Teams 队员）会继承主会话的权限模式，但实际优先级比“简单继承”更细。
...
当主会话不是 auto / dontAsk 时，子代理 mode 按下列顺序解析：

1. Agent 工具调用时显式传入的 mode
1. 子代理 frontmatter / product config 中 agent 自带的 permissionMode（写 ignore 时沿用父会话模式）
1. CLI --subagent-permission-mode
1. 环境变量 CODEBUDDY_SUBAGENT_PERMISSION_MODE
1. settings permissions.subagentPermissionMode
1. 代码内继承映射
1. 否则直接继承主会话当前模式
...
如果主会话当前是 auto 或 dontAsk，会先触发一层权限上限（ceiling）：
...
如果你在 -p、stream-json、后台代理这类不能弹审批框的环境里工作，推荐这样理解各模式：
...
- 权限规则：allow / ask / deny 的匹配语法与完整求值顺序
- Settings 配置：defaultMode、disableAutoMode、autoMode、subagentPermissionMode 等字段
- CLI 参考：--permission-mode、--subagent-permission-mode、codebuddy auto-mode 等命令
- Agent Teams：多代理协作：delegate 模式下的协作方式
- 子代理（Sub-agents）：子代理的工具与权限模式继承
- Hooks 钩子系统：用 PreToolUse / PermissionRequest / PermissionDenied 扩展权限判定
- 非交互模式：-p 流程下怎样设计权限策略
- Bash 沙箱化：在命令层额外加文件系统 / 网络隔离
- 安全：整体安全模型与最佳实践
