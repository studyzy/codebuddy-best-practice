...
> 来源: https://www.codebuddy.cn/docs/cli/mcp
...
1. 询问规则 （ask) - 使用工具前需要用户确认（覆盖允许规则）
1. 允许规则 （allow) - 允许工具使用而无需手动批准
...
1. ~/.codebuddy/mcp.json（已废弃）
1. ~/.codebuddy.json（旧版配置文件）
...
1. 团队共享配置bash# 在 .mcp.json 中使用环境变量
# 每个团队成员在本地设置环境变量
...
1. 可以通过 /mcp 命令查看 MCP 服务器配置和诊断信息
1. 诊断消息会列出所有缺失的环境变量
...
除了 MCP 配置里的静态 defer_loading，还可以在 --tools 参数或自定义代理 frontmatter 中使用 Defer(...) / NoDefer(...) 修饰符，临时改变某个工具或某组工具的延迟加载状态：
...
修饰符的优先级高于MCP 静态 defer_loading 配置；NoDefer 永远胜过 Defer。详见 工具延迟加载覆盖。
...
1. 实现 MCP 协议：使用官方 SDK 或自行实现
1. 定义工具接口：描述工具功能和参数
1. 处理请求：接收和处理来自 CodeBuddy 的请求
1. 返回结果：按 MCP 格式返回执行结果
...
- 截断降级：以下场景会直接截断内容到 MAX_MCP_OUTPUT_TOKENS * 4 字符上限，而不落盘： 响应包含图片块（避免 base64 payload 写入 .txt 文件丢失图片渲染）当前无会话上下文（避免产生孤儿文件）落盘失败（目录不可写、磁盘满等）设置了环境变量 CODEBUDDY_DISABLE_MCP_LARGE_OUTPUT_FILES=1
