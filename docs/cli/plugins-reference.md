...
> 来源: https://www.codebuddy.cn/docs/cli/plugins-reference
...
插件是一个自包含的组件目录，用于扩展 CodeBuddy 的自定义功能。插件组件包括技能 (Skills)、代理 (Agents)、钩子 (Hooks)、MCP 服务器和 LSP 服务器。
...
由于 frontmatter hooks 受安全闸门拦截，插件分发的 Skill / Agent 如果依赖 frontmatter hooks 才能正常工作，应在插件 README 中明确告知用户开启 allowUntrustedFrontmatterHooks 设置。详见 Hook 参考指南 - Frontmatter Hooks。
...
插件可以提供 Language Server Protocol (LSP) 服务器，为 AI 助手在代码库上工作时提供实时代码智能支持。
...
插件使用与其他 CodeBuddy 配置相同的作用域系统。详见 Settings。
...
清单是可选的。如果省略，CodeBuddy 会自动发现默认位置中的组件，并从目录名派生插件名称。当需要提供元数据或自定义组件路径时使用清单。
...
| userConfig | object | 启用时提示用户配置的值。详见用户配置 | 见下方 |
| channels | array | 消息注入的频道声明。详见频道 | 见下方 |
...
对于 commands、agents、skills 和 outputStyles，自定义路径会替换默认目录。如果清单指定了 commands，则不会扫描默认的 commands/ 目录。Hooks、MCP servers 和 LSP servers 对处理多个来源有不同的语义。
...
出于安全和验证目的，CodeBuddy 将市场插件复制到用户的本地插件缓存（~/.codebuddy/plugins/cache），而不是原地使用。理解此行为对于开发引用外部文件的插件很重要。
...
| Settings | settings.json | 启用插件时应用的默认配置。目前仅支持 agent 设置 |
...
1. 验证 shebang 行：第一行应为 #!/bin/bash 或 #!/usr/bin/env bash
1. 检查路径是否使用 ${CODEBUDDY_PLUGIN_ROOT}："command": "${CODEBUDDY_PLUGIN_ROOT}/scripts/your-script.sh"
1. 手动测试脚本：./scripts/your-script.sh

钩子未在预期事件上触发:

1. 验证事件名称正确（区分大小写）：PostToolUse，而不是 postToolUse
1. 检查 matcher 模式是否匹配目标工具："matcher": "Write|Edit" 用于文件操作
1. 确认钩子类型有效：command、http、prompt 或 agent
...
1. 检查命令是否存在且可执行
1. 验证所有路径使用 ${CODEBUDDY_PLUGIN_ROOT} 变量
1. 检查 MCP 服务器日志：codebuddy --debug 显示初始化错误
1. 在 CodeBuddy 之外手动测试服务器
...
> 注意: CodeBuddy 使用版本来判断是否需要更新插件。如果更改了插件代码但未更新 plugin.json 中的版本，由于缓存机制，现有用户不会看到变更。如果插件在市场目录中，可以通过 marketplace.json 管理版本，并从 plugin.json 中省略 version 字段。
...
1. 可选择将脚本中的 ${CLAUDE_PLUGIN_ROOT} 替换为 ${CODEBUDDY_PLUGIN_ROOT}
1. 可选择将 ${CLAUDE_PLUGIN_DATA} 替换为 ${CODEBUDDY_PLUGIN_DATA}
