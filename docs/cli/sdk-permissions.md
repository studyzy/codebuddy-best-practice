...
> 来源: https://www.codebuddy.cn/docs/cli/sdk-permissions
...
1. 只读任务使用 plan 模式：typescriptpermissionMode: 'plan'  // 只允许 Read、Glob、Grep
1. 结合白名单精确控制：typescriptallowedTools: ['Read', 'Glob', 'Grep'],
permissionMode: 'bypassPermissions'  // 允许的工具自动执行
1. 危险命令使用 interrupt：typescriptreturn {
  behavior: 'deny',
  message: '危险操作',
  interrupt: true  // 立即中断，不让 AI 继续尝试
};
1. 生产环境避免 bypassPermissions：该模式会跳过所有权限检查


## 相关文档 ​

- SDK 概览 - 快速入门和使用示例
- SDK Hook 系统 - 更细粒度的工具控制
