...
> 来源: https://www.codebuddy.cn/docs/cli/monitoring
...
1. 启用判定：内置 product 配置启用 或 CODEBUDDY_CODE_ENABLE_TELEMETRY / CLAUDE_CODE_ENABLE_TELEMETRY 设置为 truthy 值（1 / true / yes / on）。
1. Endpoint 优先级：OTEL_EXPORTER_OTLP_TRACES_ENDPOINT > OTEL_EXPORTER_OTLP_ENDPOINT > 内置 product 配置中的 telemetry.tracing.url。
1. Headers：env 与 product 配置合并，env 同名 key 覆盖 product。
...
- Span 默认只记录工具名称和调用 ID，不包含用户 prompt、工具参数、文件内容或源代码
