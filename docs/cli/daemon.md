...
> 来源: https://www.codebuddy.cn/docs/cli/daemon


# Daemon 模式与后台会话 ​

Daemon 模式让 CodeBuddy Code 以后台常驻服务的方式运行，不依赖终端窗口。启动后提供完整的 HTTP API 和 Web UI，随时接受请求。
...
详见 HTTP API 文档 中的 Workers & Daemon 章节。
