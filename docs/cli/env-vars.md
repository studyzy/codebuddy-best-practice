...
> 来源: https://www.codebuddy.cn/docs/cli/env-vars
...
CodeBuddy Code 支持把内部 traces 通过 OTLP 协议上报到用户自有的 Collector，环境变量遵循 OpenTelemetry 规范。详细使用方式见 Monitoring。
...
| CODEBUDDY_DISABLE_REQUEST_VALIDATION | 设置为 1 关闭 Gateway 自定义请求头校验（X-CodeBuddy-Request）。详见 HTTP API 安全 |
...
- Settings - 在 settings.json 中配置环境变量和其他设置
- CLI Reference - 命令行参数完整列表
- MCP Setup - MCP 服务器配置
- 子代理 - 子代理存储目录说明
