...
> 来源: https://www.codebuddy.cn/docs/cli/channels
...
- 支持的 Channel：微信、Telegram、Discord 以及 fakechat 设置
- 快速开始：使用微信 channel 快速体验
- 安全机制：发送者白名单和配对流程
- 组织管控：为团队和企业启用 channel
- Channel 与其他功能的对比

要构建自己的 channel，请参阅 Channels 参考。
...
需要安装插件。查看完整 Telegram 插件源码。

1. 在 Telegram 中通过 BotFather 创建一个 bot，获取 token
1. 安装插件：/plugin install telegram@claude-plugins-official
1. 配置 token：/telegram:configure <token>
1. 启动：codebuddy --channels plugin:telegram@claude-plugins-official
1. 向 bot 发消息获取配对码，执行 /telegram:access pair <code> 完成配对


### Discord ​

需要安装插件。查看完整 Discord 插件源码。

1. 在 Discord 开发者门户 创建应用和 bot
1. 启用 Message Content Intent，邀请 bot 到服务器
1. 安装插件：/plugin install discord@claude-plugins-official
1. 配置 token：/discord:configure <token>
1. 启动：codebuddy --channels plugin:discord@claude-plugins-official
1. DM bot 获取配对码，执行 /discord:access pair <code> 完成配对
...
1. 向 bot 发送任意消息
1. bot 回复配对码
1. 在 CodeBuddy Code 中确认配对码
1. 发送者 ID 加入白名单

白名单同时控制权限中继。通过 channel 回复的人可以审批或拒绝工具调用，因此只将信任的发送者加入白名单。
...
- 构建自己的 Channel：为还没有插件的系统创建 channel
